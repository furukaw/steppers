...
and subst_cont_in (cont_in : cont_in) (pairs : (string * v) list) : cont_in =
  match cont_in with
  | FId -> FId
  | FApp2 (e1, cont_in) -> FApp2 (subst e1 pairs, subst_cont_in cont_in pairs)
  | FApp1 (v2, cont_in) -> FApp1 (subst_v v2 pairs, subst_cont_in cont_in pairs)
  | FOp (name, cont_in) -> FOp (name, subst_cont_in cont_in pairs)
  | FWith (e2, cont_in) -> FWith (subst e2 pairs, subst_cont_in cont_in pairs)

and subst_cont_out (cont_out : cont_out) pairs : cont_out = match cont_out with
  | GId -> GId
  | GHandle (h, (cont_in, cont_out)) ->
    GHandle (subst_h h pairs, (subst_cont_in cont_in pairs,
                               subst_cont_out cont_out pairs))
