...
let rec subst_value (v : v_t) (pairs : (string * v_t) list) : v_t = match v with
...
and subst_handler ({return; ops} : h_t) (pairs : (string * v_t) list) : h_t =
...
(* e の中の自由変数を値に置換する *)
and subst (e : e_t) (pairs : (string * v_t) list) : e_t = match e with
...
let rec record_var_name : e_t -> unit = function
...
and record_var_name_handler : h_t -> unit = fun {return; ops} ->
...
and record_var_name_value : v_t -> unit = function
