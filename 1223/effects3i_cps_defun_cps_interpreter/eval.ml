...
and apply_handler (cont_last : cont_in) (h : h) (a : a) (cont_out : cont_out) : a =
  ...
  | OpCall (name, v, cont_in') ->
    begin match search_op name h with
      | None ->
        cont_out (OpCall (name, v, FCall (cont_last, h, cont_in')))
      | Some (x, k, e) ->
...
and apply_in (cont_in : cont_in) (v : v) (cont_out : cont_out) : a =
  ...
  | FWith (e2, cont_in) -> let v1 = v in
    ...
    eval e2 FId (fun a -> apply_handler cont_in h a cont_out)
  | FCall (cont_last, h, cont_in') ->
    apply_in cont_in' v (fun a -> apply_handler cont_last h a cont_out)
