...
and apply_handler (cont_last : k) (h : h) (a : a) (k2 : k2) : a =
  match a with
  | Return v ->
  ...
  | OpCall (name, v, k') ->
    begin match search_op name h with
      | None ->
        k2 (OpCall (name, v, FCall (cont_last, h, k')))
      | Some (x, k, e) ->
...
and apply_in (k : k) (v : v) (k2 : k2) : a =
  ...
  | FWith (e2, k) -> let v1 = v in
    ...
    eval e2 FId (fun a -> apply_handler k h a k2)
  | FCall (cont_last, h, k') ->
    apply_in k' v (fun a -> apply_handler cont_last h a k2)
