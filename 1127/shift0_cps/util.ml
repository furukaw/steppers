...
let rec subst_v (v : v) (pairs : (string * v) list) : v = match v with
  ...
  | VCont (cont, (x, e)) ->
    if List.mem_assoc x pairs then v else VCont (cont, (x, subst e pairs))
...
and subst (e : e) (pairs : (string * v) list) : e = match e with
  ...
  | Shift0 (x, e) ->
    if List.mem_assoc x pairs then e else Shift0 (x, subst e pairs)
  | Reset0 (e) -> Reset0 (subst e pairs)
...
let rec record_var_name : e -> unit = function
  ...
  | Shift0 (x, e) -> add_var_name x; record_var_name e
  | Reset0 (e) -> record_var_name e

and record_var_name_value : v -> unit = function
  | Var (x) -> ()
  | VFun (x, e) -> add_var_name x; record_var_name e
  | VCont (_, (x, e)) -> add_var_name x; record_var_name e
