  ...
  | Shift (x, e1) ->
    begin match cont_out with
      ...
      | (GReset (cont_in', cont_out')) ->
        ...
        let x_cont_in = Cont (cont_in, new_var) in
...
and apply_in (cont_in : cont_in) (v : v) (cont_out : cont_out): v =
  ...
  | FApp2 (e1, cont_in2) -> let v2 = v in
    eval e1 (FApp1 (v2, cont_in2)) cont_out
  | FApp1 (v2, cont_in1) -> let v1 = v in
    begin
      match v1 with
      | Fun (x, e_fun) ->
      ...
      | Cont (cont_in', x) ->
        ...
        memo redex reduct (cont_in1, cont_out);
        apply_in cont_in' v2 (GReset (cont_in1, cont_out))
