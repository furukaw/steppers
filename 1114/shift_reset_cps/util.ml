...
let rec subst (term : t) (pairs : (string * v) list) : t = match term with
  ...
  | Fun (x, t) ->
    if List.mem_assoc x pairs then term else Fun (x, subst t pairs)
  | App (t1, t2) ->
...
and subst_v (v : v) (pairs : (string * v) list) : v = match v with
  ...
  | VFun (f, (x, t)) ->
    if List.mem_assoc x pairs then v else VFun (f, (x, subst t pairs))
  | VCont (cont, (x, t)) ->
    if List.mem_assoc x pairs then v else VCont (cont, (x, subst t pairs))
...
and collect_v : v -> unit = function
  | Var (_) -> ()
  | VFun (_, (x, t)) -> add x; collect t
  | VCont (_, (x, t)) -> add x; collect t
